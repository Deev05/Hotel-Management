...
namespace App\Modules\Sopusertickets\Controllers;
use App\Modules\Sopusertickets\Controllers\Notification;
use App\Controllers\BaseController;
use App\Models\CommonModel;

class Sopusertickets extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->table = " sop_application_user_tickets ";
        helper(['form', 'url']);
    }
    ...
    public function index()
    {
        ...
        $data['page_headline'] = 'Ticket Management';
        $order[0] = "id";
        $order[1] = "DESC";
        ...
        $query = "SELECT * FROM sop_application_user_tickets";
        $allorders   = $this->CommonModel->custome_query($query);
        $data['all_tickets'] = count($allorders);
        
        $query = "SELECT * FROM sop_application_user_tickets where status = 0";
        $allorders   = $this->CommonModel->custome_query($query);
        $data['opened'] = count($allorders);
        
        $query = "SELECT * FROM sop_application_user_tickets where status = 1";
        $allorders   = $this->CommonModel->custome_query($query);
        $data['in_progress'] = count($allorders);
        
        $query = "SELECT * FROM sop_application_user_tickets where status = 2";
        $allorders   = $this->CommonModel->custome_query($query);
        $data['responded'] = count($allorders);
        
        $query = "SELECT * FROM sop_application_user_tickets where status = 3";
        $allorders   = $this->CommonModel->custome_query($query);
        $data['resolved'] = count($allorders);

        echo view('App\Modules\Admin\Views\header', $data);
        echo view('App\Modules\Admin\Views\sidebar', $data);
        echo view('App\Modules\Sopusertickets\Views\index', $data);
    ...
    public function get_data()
    {
        ...
        $totalRecordsSql = "SELECT * FROM sop_application_user_tickets $where;";
        ...
        $sql = "SELECT *";
        $sql .= " FROM sop_application_user_tickets $where";
        ...
        foreach ($result as  $row) 
        {
            ...
            $data['user_name']       = $user_details->firstname." ".$user_details->lastname;
    ...
    public function single()
    {
        ...
        $data['ticket_details'] = $this->CommonModel->get_single("sop_application_user_tickets",$filter);
        ...
        echo view('App\Modules\Admin\Views\header', $data);
        echo view('App\Modules\Admin\Views\sidebar', $data);
        echo view('App\Modules\Sopusertickets\Views\single', $data);
        echo view('App\Modules\Admin\Views\footer', $data);
    }
    ...
    public function change_ticket_status()
    {
        ...
        $update = $this->CommonModel->update_data('sop_application_user_tickets', $update_data, $filter);
        ...
        if ($update != false) {
            ...
            $filter = array("id" => $ticket_id);
            $ticket_data = $this->CommonModel->get_single("sop_application_user_tickets",$filter);
            ...
            $data['ticekt_data'] = $this->CommonModel->get_single("sop_application_user_tickets",$filter);
            $data['title']=$title;
            $data['message']=$message;
            $data['user']=$this->CommonModel->get_single("user_master",array('id'=>$data['ticekt_data']->user_id));
