...
?> 
            ...
            <div class="container-fluid">
                <div class="text-left">
                    <button type="submit" class="btn btn-info reorder_button" name="reorder_button">Reorder Categories</button>
                </div>
                <br/>
                <!-- Reorder Categories modal content -->
                                    ...
                                    <div class="myadmin-dd dd" id="nestable">
                                        <ol class="dd-list">
                                            ...
                                            <?php 	 
                                                ...
                                                $query = "select * from categories where is_deleted = 0 and status = 1 order by orders asc";
                ...
                <div class="card">
                    <div class="card-body">
                    <?php
						$form_location = base_url()."/categories/add_category";
					?>
                                ...
                                <form id="update_category_form" method="POST" enctype="multipart/form-data">
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group col-md-12 input-group custom-file">
                                            <input type="file" class="custom-file-input" id="edit_category_image" name="edit_category_image">
                                            <label class="custom-file-label" for="inputGroupFile04">Choose file</label>  
                                        </div>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Close</button>
                                        <button type="submit" class="btn btn-danger waves-effect waves-light update_category">Save</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    var table = $('.show-child-rows').DataTable({
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render" : function(data){
                             var image_url = "<?= base_url("uploads/categories") ?>/" + data;
                    ...
                    },
                    {
                        ...
                        "data": "id",
                        "render": function(data) {
                            var edit_url = "<?= base_url("categories/create") ?>/" + data;
                            
                            return '<div class="btn-group">' +
                                '<a href="javascript:void(0)" class="btn btn-info category_modal">' +
    ...
    $(document).on('click', '.status', function() {
        ...
        $.ajax({
            method: "POST",
            url: "<?= base_url(); ?>/categories/change_status/" + id,
    ...
    $(document).on('click', '.category_modal', function(e) {
        var id              = table.row($(this).closest('tr')).data().id;
        var category_name    = table.row($(this).closest('tr')).data().name;
        var image           = table.row($(this).closest('tr')).data().image;
        var image_url       = '<?php echo base_url(); ?>/uploads/categories/'+image;
    ...
    $(document).on('click', '.reorder_button', function(e) {
        ...
        var updateOutput = function(e) {
                ...
                if (window.JSON) 
                {
                    ...
                    $.ajax({
                                url: '<?= base_url(); ?>/categories/reorder_list',
                                enctype: 'multipart/form-data',
