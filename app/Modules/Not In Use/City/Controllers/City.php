...
class City extends BaseController {
    ...
    public function index()
    {
        ...
        $this->permissionCheckNormal('view_city','normal');

        $data['page_title']     = 'City';
        $data['page_headline']  = 'City';
        ...
        echo view('App\Modules\City\Views\index', $data);
    ...
    public function get_city()
    {
        ...
        $totalRecordsSql = "SELECT * FROM city $where;";
    ...
    public function add_city()
    {
        ...
        ////////////////////////Check Permission/////////////////
        $check = $this->permissionCheck('add_city');
        ...
        $insert = $this->CommonModel->common_insert_data('city', $insert_data);
        
        if ($insert != false) 
        {

            $activity_title = "New City created named " . $name;
            $this->addActivityLog($activity_title);

            $message = ['status' => '1', 'message' => 'New City has been added!'];
            return json_encode($message);
        } 
        else 
        {
    ...
    public function delete()
    {
        ...
        ////////////////////////Check Permission/////////////////
        $check = $this->permissionCheck('delete_city');
        $check_resp = json_decode($check);
        $status = $check_resp->status;
        if($status == 0)
        {
        ...
        if ($update != false) 
        {
            $activity_title = "City Removed " . $id;
            $this->addActivityLog($activity_title);

            $message = ['status' => '1', 'message' => 'City has been deleted!'];
            return json_encode($message);
        } 
        else 
        {
    ...
    public function update_city()
    {
        ...
        ////////////////////////Check Permission/////////////////
        $check = $this->permissionCheck('update_city');
        $check_resp = json_decode($check);
        $status = $check_resp->status;
        if($status == 0)
        {
        ...
        if ($update != false) 
        {
            $activity_title = "City Updated " . $id;
            $this->addActivityLog($activity_title);
