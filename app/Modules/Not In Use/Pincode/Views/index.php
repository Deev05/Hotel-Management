...
?>
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-body">
                    <?php
						$form_location = base_url()."/pincode/add_pincode";
					?>
                ...
                <div class="row">
                    ...
                    <!-- Edit pincode modal content -->
                    <div id="responsive-modal" class="modal fade edit_pincode" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form id="update_pincode_form" method="POST" enctype="multipart/form-data">
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group col-md-12">
                                            <input type="hidden" class="form-control" name="pincode_id" id="pincode_id" value="">
                                        </div>
                
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Close</button>
                                        <button type="submit" class="btn btn-danger waves-effect waves-light update_pincode">Save</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    var table = $('.show-child-rows').DataTable({
                ...
                "paging": true,
                "ajax": {
                    "url": "<?= base_url() ?>/pincode/get_pincode",
                    "type": 'POST',
                ...
                "columns": [{
                    ...
                    
                    {
                        ...
                        "data": "id",
                        "render": function(data) {
                            var edit_url = "<?= base_url("pincode/create") ?>/" + data;
                            
                            return '<div class="btn-group">' +
                                '<a href="javascript:void(0)" class="btn btn-info pincode_modal">' +
    ...
    $(document).on('click', '.delete_button', function() {
        ...
        $.ajax({
            method: "POST",
            url: "<?= base_url(); ?>/pincode/delete/" + id,
            success: function(message) {
                table.draw();
                obj = JSON.parse(message);
                var status = obj.status;
    ...
    $('#add_form').validate({ 
		...
		submitHandler: function(form) 
		{
			...
			$.ajax({
				...
				url: '<?php echo base_url(); ?>/pincode/add_pincode',
    ...
    $(document).on('click', '.pincode_modal', function(e) {
        var id              = table.row($(this).closest('tr')).data().id;
    ...
    $("#update_pincode_form").validate({
        ...
        submitHandler: function(form) {
            $.ajax({
                url: '<?= base_url(); ?>/pincode/update_pincode',
                enctype: 'multipart/form-data',
                data: new FormData(form),
                processData: false,
                contentType: false,
